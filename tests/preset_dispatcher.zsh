...
stderr_file="$(mktemp)"
"$REINA_BIN" run parallel-processing-drums >/dev/null 2>"$stderr_file"
exit_code=$?
...
assert_contains "$stderr_output" "parallel-processing-drums" "error menciona el slug" || exit 1

stderr_file="$(mktemp)"
"$REINA_BIN" run parallel-processing-drums --dry-run >/dev/null 2>"$stderr_file"
exit_code=$?
stderr_output="$(<"$stderr_file")"
rm -f "$stderr_file"
...
run_json_output="$("$REINA_BIN" --json run parallel-processing-drums 2>/dev/null)"
