...
stderr_file="$(mktemp)"
"$REINA_BIN" run upright-bass >/dev/null 2>"$stderr_file"
exit_code=$?
...
assert_contains "$stderr_output" "upright-bass" "error menciona el slug" || exit 1

stderr_file="$(mktemp)"
"$REINA_BIN" run upright-bass --dry-run >/dev/null 2>"$stderr_file"
exit_code=$?
stderr_output="$(<"$stderr_file")"
rm -f "$stderr_file"
