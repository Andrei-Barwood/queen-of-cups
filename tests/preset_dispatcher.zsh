...
stderr_file="$(mktemp)"
"$REINA_BIN" run acoustic-guitar-wet >/dev/null 2>"$stderr_file"
exit_code=$?
...
assert_contains "$stderr_output" "acoustic-guitar-wet" "error menciona el slug" || exit 1

stderr_file="$(mktemp)"
"$REINA_BIN" run acoustic-guitar-wet --dry-run >/dev/null 2>"$stderr_file"
exit_code=$?
stderr_output="$(<"$stderr_file")"
rm -f "$stderr_file"
...
run_json_output="$("$REINA_BIN" --json run acoustic-guitar-wet 2>/dev/null)"
